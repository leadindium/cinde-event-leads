...
import { mockLeads, mockCompanyConfig } from '../utils/mock-data';

interface LeadStore {
  leads: Lead[];
  config: CompanyConfig;
  // Lead CRUD
  addLead: (lead: Omit<Lead, 'id' | 'capturedAt' | 'updatedAt' | 'status' | 'photos' | 'documentsSent' | 'tags' | 'rating' | 'notes'> & Partial<Pick<Lead, 'status' | 'photos' | 'documentsSent' | 'tags' | 'rating' | 'notes'>>) => Lead;
...
export const useLeadStore = create<LeadStore>((set, get) => ({
  ...
  addCustomTag: (tag) =>
    set((s) => ({
      config: { ...s.config, customTags: [...s.config.customTags, tag] },
    })),
  ...
  addDocument: (doc) =>
    set((s) => ({
      config: { ...s.config, documents: [...s.config.documents, doc] },
    })),
